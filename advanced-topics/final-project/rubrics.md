...
| **Testing Requirements** | **20+ functional tests** covering multiple categories such as Smoke, Logic, Interaction, and Simulated Input. Includes **Negative Testing** and edge cases. Code coverage for testing is `TBD`<sup>[1]</sup> or greater. | 10–15 tests covering 2-3 categories. Tests core mechanics and the "Happy Path" with specific assertions. | Fewer than 10 tests; logic coverage is limited to basic use cases with weak assertions. |
...
[1] - The actual test coverage will be set to be fair to the entire class. The initial goal will be 70% -- however, if there are many students below this I will reduce this to a lower goal.
...
| **Teamwork** (Individual grade) | Both team members completed contributed equally to presentation and project development effort. | One team member contributed more. | Signifcant disparity in team effort. |
